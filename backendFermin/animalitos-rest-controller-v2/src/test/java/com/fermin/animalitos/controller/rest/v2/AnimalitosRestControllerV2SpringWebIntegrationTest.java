...
public class AnimalitosRestControllerV2SpringWebIntegrationTest {
	...
	AnimalitosRestControllerV2SpringWebIntegrationTest(@Autowired MockMvc clienteWeb){
	...
	void altaGuay() throws Exception {
		...
		DatosAnimalito datosQueDevuelveElServicio = DatosAnimalito.builder().id(33L).nombre(nombre).color("morado").tipo(tipo).edad(edad).build();
		...
		objetoJson.put("age", edad);
		objetoJson.put("tipo", tipo);
		objetoJson.put("color", "morado");
		
		ResultActions resultado = clienteWeb.perform(MockMvcRequestBuilders.post("/api/v2/animalitos")
		...
		resultado.andExpect(jsonPath("$.age").value(edad));
		resultado.andExpect(jsonPath("$.color").value("morado"));
		resultado.andExpect(jsonPath("$.id").value(33));
	}
	...
	@Test
	@DisplayName("petición http post, mandando un objeto JSON inválido. Sin color")
	void altaRuina2() throws Exception {
		int edad = 1;
		TipoAnimalito tipo = TipoAnimalito.PERRO;
		...
		objetoJson.put("age", edad);
		objetoJson.put("tipo", tipo);
		objetoJson.put("nombre", "Firulais");
